...
export type AudioRatingPluginEvents = BasePluginEvents & {
  'points-change': [newPoints: RatingPoint[]]
...
class Polyline extends EventEmitter<{
  ...
  addPolyPoint(relX: number, relY: number, refPoint: RatingPoint) {
    const { svg } = this
...
class AudioRatingPlugin extends BasePlugin<AudioRatingPluginEvents, AudioRatingPluginOptions> {
  ...
  private points: RatingPoint[]
  private throttleTimeout: ReturnType<typeof setTimeout> | null = null
  private rating = 1

  /**
   * Create a new AudioRating plugin.
   */
  ...
  /**
   * Add an envelope point with a given time and rating.
   */
   ...
   */
  public setPoints(newPoints: RatingPoint[]) {
  ...
  /**
   * Set the envelope rating. 0..1.
   */
  public setRating(floatValue: number) {
  ...
  private addPolyPoint(point: RatingPoint, duration: number) {
    this.polyline?.addPolyPoint(point.time / duration, point.rating, point)
  }
